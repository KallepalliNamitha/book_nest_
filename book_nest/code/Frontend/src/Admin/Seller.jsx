...
const Seller = () => {
  ...
  // const [showDetail, setShowDetail] = useState(false);
   ...
   useEffect(() => {
    axios.get(`http://localhost:5001/sellers`)
      .then((response) => {
...
const deleteData = (taskId) => {
    axios.delete(`http://localhost:5001/sellerdelete/${taskId}`);
    window.location.assign('/sellers');
    alert('User is deleted');
  };
  const deleteitem = (taskId) => {
    axios.delete(`http://localhost:5001/useritemdelete/${taskId}`);
  ...
  const fetchUserBikeData = (userId) => {
    ...
    axios.get(`http://localhost:5001/getitem/${userId}`)
                            ...
                            return (
                              ...
                              >
                                <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                                  <div>
                                    <img src={`http://localhost:5001/${item?.itemImage}`} alt={`${item.itemtype} Image`} style={{ height: "80px",width:"120px" }} /> <br/>
                                  </div>
                                  <div>
                                    <p>Product Name:</p>
